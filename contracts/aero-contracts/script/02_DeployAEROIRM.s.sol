...
/// @title 02_DeployAEROIRM
/// @notice Step 2: Deploy a KinkIRM for AERO borrow market.
///
...
///
/// @dev Run:
///      source .env && forge script script/02_DeployAEROIRM.s.sol \
...
contract DeployAEROIRM is Script {
    ...
    function run() external {
        ...
        console.log("\n=== STEP 2 COMPLETE: AERO IRM Deployed ===");
        console.log("KINK_IRM_AERO=%s  (Base=0%%, Kink(85%%)=16%%, Max=750%%)", aeroIrm);
        console.log("\nPaste into .env, then run 03_DeployAEROBorrowVault.s.sol");
    }
}
