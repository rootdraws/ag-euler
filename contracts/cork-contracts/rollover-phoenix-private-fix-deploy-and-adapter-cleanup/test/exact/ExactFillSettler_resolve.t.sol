...
contract ExactFillSettler_resolve is ExactFillSettlerTestBase {
    // ═══════════════════════════════════════════════════════════════
    //  Helpers
    // ═══════════════════════════════════════════════════════════════
    ...
    {
        (, od,) = _createExactOrder(user, DEFAULT_ORDER_SIZE);
    ...
    function test_resolve_wellFormed_outputsReflectOrderSizeAndPremium() external view {
        (IOriginSettler.OnchainCrossChainOrder memory onchain, OrderData memory od) = _wellFormedOrder();

        IOriginSettler.ResolvedCrossChainOrder memory resolved = settler.resolve(onchain);

        // Rollover leg: amount == orderSize
    ...
    function test_resolve_wellFormed_originDataIsByteExact() external view {
        ...
        // resolve() builds a synthetic GaslessCrossChainOrder and encodes it as originData
        ...
        // destinationSettler should be the settler address
