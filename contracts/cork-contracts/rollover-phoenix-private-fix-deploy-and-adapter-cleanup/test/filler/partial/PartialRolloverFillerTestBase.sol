...
/// @title PartialRolloverFillerTestBase
/// @notice Shared test infrastructure for the Partial-binding `PartialRolloverFiller` BTT suite. Mirrors
///         `ExactRolloverFillerTestBase` one-for-one, but:
///           - binds the signing helpers to `partialSettler.domainSeparator()`,
///           - orders are built with `allowPartialFills = true` (Partial path requirement —
///             `_validateOpen` rejects `false`).
...
abstract contract PartialRolloverFillerTestBase is BaseTestFiller {
    ...
    /// @notice Build a canonical Partial rollover order with two outputs (rollover + premium), a
    ...
    ///         envelope. `allowPartialFills = true` (required by `PartialFillSettler._validateOpen`).
    /// @param uw The underwriter wallet (signer for both `openFor` and `cellarSignature`).
    /// @param orderSize Rollover order size.
    /// @param destination_ dstCST recipient for the rollover leg filler data (unused — retained for
    ///        signature parity with the Exact base).
    ...
    /// @notice Prepare a caller's preconditions for a filler `execute` call against the Partial
    ///         binding. Mirrors the runtime checklist: premium deposited for `actor`, settler
    ///         authorised as ERC-6909 operator, filler authorised as ERC-6909 premiumFiller, and
    ///         srcCST approved to the filler.
